...
type Group = { id: string; name: string; kind: string; status: string; templates: Array<AssistantTemplate & { installedId: string | null }> };
...
export function TeamCatalog({ close, saved, notify }: { close(): void; saved(): void; notify: Notify }) {
  const [groups, setGroups] = useState<Group[]>([]);
  const [source, setSource] = useState(new URLSearchParams(location.search).get("source") ?? "workspace");
  const [ids, setIds] = useState<string[]>([]);
  const [confirmed, setConfirmed] = useState(false);
  const [provider, setProvider] = useState("subscription");
  const [busy, setBusy] = useState(false);
  const [firstBrief, setFirstBrief] = useState(true);
  const [failure, setFailure] = useState("");
  async function load() { try { setGroups(await api("/api/team/catalog")); setFailure(""); } catch (e) { setFailure(errorText(e)); } }
  useEffect(() => { void load(); }, []);
  const group = groups.find((g) => g.id === source) ?? groups[0];
  useEffect(() => { setIds(group?.templates.filter((t) => !t.installedId).slice(0, 3).map((t) => t.id) ?? []); setConfirmed(group?.kind === "demo"); }, [group]);
  const runtime = group?.templates.some((t) => ids.includes(t.id) && t.mode === "runtime");
  async function install() {
    if (!group) return; setBusy(true);
    try {
      const result = await api<{ created: number; results: Array<{ error?: string }> }>("/api/team/install", { method: "POST", body: JSON.stringify({ connectorId: group.id, templateIds: ids, runtimePolicyConfirmed: confirmed, providerPolicy: provider, startFirstBrief: firstBrief }) });
