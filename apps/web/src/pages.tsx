...
export function ConnectionsPage({ notify }: NotifyProps) {
  const [data, setData] = useState<{ connectors: Connector[]; catalog: Array<{ id: string; displayName: string; capabilities: string[] }> } | null>(null);
  const [adding, setAdding] = useState(false);
  const [syncing, setSyncing] = useState<string | null>(null);
  async function load() { setData(await api("/api/connectors")); }
  useEffect(() => { load().catch((error) => notify(error.message, "error")); }, []);
  async function sync(id: string) { setSyncing(id); try { await api(`/api/connectors/${id}/sync`, { method: "POST" }); await load(); notify("Connection synchronized", "success"); } catch (error) { notify(error instanceof Error ? error.message : "Sync failed", "error"); await load(); } finally { setSyncing(null); } }
...
function AddConnection({ catalog, close, added, notify }: { catalog: Array<{ id: string; displayName: string; capabilities: string[] }>; close(): void; added(): void; notify: NotifyProps["notify"] }) {
  const [kind, setKind] = useState(catalog[0]?.id ?? "openclaw-cli");
