...
COUNT = 19
COUNTS = {"folder-full": COUNT, "folder-alias": COUNT, "folder-alias-new": COUNT}
SUMMARIES = {mode: f"PASS: {COUNT} folder scope assertions ({mode}; synthetic owner-selected folder)." for mode in COUNTS}
READY = "INSTRUMENTATION_STATUS: stream=PHONE_FOLDER_SCOPE_BIOMETRIC_READY create"
...
class Transcript:
    ...
    def __init__(self, mode="folder-full"):
        if mode not in COUNTS:
            raise bio.ProbeFailure("invalid_probe_mode")
        self.mode = mode
        self.expected_ready = 1
        self.pending = False
        self.ready = 0
        self.passed = False
        self.terminal = False
    ...
    def feed(self, line):
        ...
        if line == READY:
            ...
            if stage not in bio.FAILURE_STAGES | {"folder_scope_probe"} or "stage" in self.failure or self.passed or self.pending:
...
def source_facts(deadline):
    ...
    return {"gitHead": commit, "workingSourcesSha256": digest.hexdigest(), "driverSha256": sha(Path(__file__).read_bytes()),
            "probeSha256": sha((ROOT / "app/src/androidTest/java/io/github/quintond/orchestrator/phonecontrol/FolderScopeProbe.java").read_bytes())}
...
def draft_snapshot(prefix, deadline):
    raw = bio.capture(prefix + ["exec-out", "run-as", FIXTURE, "find", "files/drafts", "-type", "f"], min(deadline, time.monotonic() + 5)).decode("ascii")
    files = {}
    for line in raw.splitlines():
        if not re.fullmatch(r"files/drafts/[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.draft\.txt", line) or len(files) >= 256:
            raise bio.ProbeFailure("unexpected_fixture_draft_file")
        files[line] = bio.capture(prefix + ["exec-out", "run-as", FIXTURE, "cat", line], min(deadline, time.monotonic() + 5), cap=8193)
    return files


def run_probe(adb, serial, resource_id, record, mode="folder-full"):
    ...
    try:
        ...
        draft_before = draft_snapshot(prefix, deadline)
        record["draftsBefore"] = {name: facts(data) for name, data in draft_before.items()}
        record["stage"] = "instrumentation"
        mode_arguments = ["-e", "probeMode", mode]
        child = bio.BoundedProcess(prefix + ["shell", "am", "instrument", "-w", "-r", "-e", "folderScopeResourceId", resource_id] + mode_arguments + [bio.RUNNER])
        attempted = True
        record["attempts"] = 1
        used = set()

        def finger(marker):
            if marker != "create" or record["sensorEventsAttempted"]:
                raise bio.ProbeFailure("unexpected_sensor_request")
            print("PHONE_FOLDER_SCOPE_BIOMETRIC_READY create", flush=True)
            ...
            print("PHONE_FOLDER_SCOPE_SENSOR_SENT create", flush=True)
            record["stage"] = "instrumentation"
        ...
        target_check = "targetUnchanged"
        record[target_check] = after["target"] == before["target"]
        ...
        draft_after = draft_snapshot(prefix, deadline)
        record["draftsAfter"] = {name: facts(data) for name, data in draft_after.items()}
        added = set(draft_after) - set(draft_before)
        if any(draft_after.get(name) != data for name, data in draft_before.items()) or len(added) != (1 if mode == "folder-full" else 0):
            raise bio.ProbeFailure("independent_draft_inventory_failed")
        if mode == "folder-full" and draft_after[next(iter(added))] != b"Synthetic folder draft QA\r\nComplete owner-reviewed draft.\n":
            raise bio.ProbeFailure("independent_draft_text_failed")
        record["newDraftsVerified"] = len(added)
...
def main():
    ...
    parser.add_argument("--mode", choices=tuple(COUNTS), default="folder-full")
    args = parser.parse_args()
    record = {"schemaVersion": 1, "probe": "android.folder-drafts.v1", "passed": False}
    ...
    try:
        ...
        print("PHONE_FOLDER_SCOPE_FAILED evidence_new_file_required", flush=True)
        return 1
    print(SUMMARIES[args.mode] if record["passed"] else "PHONE_FOLDER_SCOPE_FAILED " + record.get("failureCode", "unknown"), flush=True)
