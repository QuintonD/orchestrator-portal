...
8. **Retrieve knowledge:** browse indexed documents, inspect their source and modification time, and search selected local folders, Obsidian vaults, Notion pages or an explicitly connected gbrain instance. Correct documents in their original source and refresh the connection. Index bounds and unavailable content appear as coverage warnings. See [knowledge setup](knowledge.md).
9. **Work with Grok Bot:** prepare an editable task, copy it into the Grok Bot app, preview its returned JSON or text file and explicitly import a claimed report. Review it in Reports and copy corrections back to Grok Bot. This is a manual handoff; the portal does not send Grok Bot messages or monitor execution. See [Grok Bot setup](grok-bot.md).
10. **Continue work in T3 Code:** add its workspace URL, prepare an editable project handoff, remove unrelated context, copy it, then open the independent workspace. No undocumented T3 execution or session API is used.
11. **Personalize and recover:** reorder, resize, and hide supported dashboard widgets; save them to the local profile; undo the last saved layout in Settings. Arbitrary widget code and assistant-authored layout changes are not enabled.
...
| Obsidian vault | Selected vault Markdown indexing, search and document inspection | Hidden settings and plugins excluded. Nested notes, escaped filenames, deletion and source bounds covered by fixtures and browser journeys. |
| Notion pages | Selected pages and ordinary nested text blocks, read-only | API v2026-03-11 fixtures cover pagination, bounds, partial coverage, access revocation and token handling. No live account validation yet. Child pages, synced blocks, databases and attachments are excluded. |
| Grok Bot handoff | Saved task text, manual JSON/text result preview and claimed report import | No automated chat or runtime monitoring. Tests cover import identity, replay, hostile content, report review and the Android JSON picker. |
